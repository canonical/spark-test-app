...
from core.domain import DeployMode, Flavour, SparkServiceAccountInfo
...
class BaseEventHandler(Object, WithLogging):
    ...
    def get_app_status(
        ...
        if self.context.config.flavour == Flavour.KAFKA.value and not self.context.kafka:
        ...
        if (
            self.context.config.deploy_mode == DeployMode.CLUSTER.value
            and not k8s_manager.is_s3_configured()
            and not k8s_manager.is_azure_storage_configured()
        ):
            return Status.MISSING_OBJECT_STORAGE_BACKEND.value
