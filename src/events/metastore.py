...
class MetastoreEvents(BaseEventHandler, WithLogging):
    ...
    def __init__(self, charm: CharmBase, context: Context, workload: KafkaAppWorkloadBase):
        ...
        self.framework.observe(
            self.database_provider.on.database_created, self._on_db_created
        )
        self.framework.observe(
            self.database_provider.on.endpoints_changed, self._on_db_created
        )
        self.framework.observe(
    ...
    def _on_db_created(self, event: RelationEvent) -> None:
        ...
        if metastore := self.context.metastore:
            ...
            self.workload.write(
                hive_site.contents, str(self.workload.paths.hive_site)
            )

        self.workload.set_environment({
            "HIVE_TABLE": f"relation{event.relation.id}"
        })
